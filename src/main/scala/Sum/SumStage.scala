...
/** SumStage
  ...
  */
class SumStage(val bitWidth : Int, val fracWidth : Int, val stages : Int,
  ...
  val io = new Bundle {
    ...
    val zi     = Vec.fill(stages + 1){Fixed(INPUT, bitWidth, fracWidth)} // Pipelined inputs (newest is 0)
  ...
  val sumLModule = Module(new SumL(bitWidth, fracWidth, stages - 1, isNORMA))
  ...
  val sumRModule = Module(new SumR(bitWidth, fracWidth, stages - 1, dictSize))
  sumRModule.io.vi := io.vi
  sumRModule.io.alpha := io.alphai
...
class SumStageTests(c : SumStage) extends Tester(c) {
  ...
  val cycles = 2*c.stages
  val expectSum = ArrayBuffer.fill(c.stages)(0)
  val expectzp  = ArrayBuffer.fill(c.stages)(0)
  val expectwD  = ArrayBuffer.fill(c.stages)(0)
  ...
  for (cyc <- 0 until cycles) {
    ...
    val zi     = ArrayBuffer.fill(c.stages + 1)(r.nextInt(1 << (c.bitWidth/2)))
    ...
    if (cyc < cycles - c.stages) {
      ...
      for (i <- 0 until c.stages) {
        if (addToDicts(cyc + i) == 1) 
          sum = ((forget*sum) >> c.fracWidth) + ui(c.stages - 1 - i)
        else {
          if (c.isNORMA)
